  ...
  return (
    <div className="py-14 relative overflow-hidden">
      ...
      <div data-aos="fade-up" className="wrapper text-center space-y-3 mb-8">
        <h2 className="text1 text-primary2">What Our Clients Say</h2>
        <p className="desc max-w-4xl mx-auto">
          Teams across real estate, education, e-commerce, and healthcare trust
          LinkAI to ship websites, apps, AI software, and chatbots that deliver
          real business outcomes—not just polished demos.
        </p>
      </div>
      <Swiper
        data-aos="fade-up"
        ...
        {testimonials.concat(testimonials, testimonials).map((item, index) => (
          <SwiperSlide key={index} className="text-white">
            <div className="flex flex-col justify-center items-center">
              <img
                src={item.image}
                className="z-[1] w-[6rem] h-[6rem] min-w-[6rem] object-cover rounded-full border-4 border-primary"
                alt=""
                loading="lazy"
              />
              <div className="content space-y-2 md:min-h-[20rem] lg:min-h-[15rem] flex flex-col justify-between bg-primary text-center -translate-y-[3rem] pt-[4rem] rounded-xl p-6">
                <p className="desc">{item.review}</p>
                <div className="space-y-1 pt-4">
                  <p className="text-white text4 !font-medium">
                    {item.name}
                  </p>
